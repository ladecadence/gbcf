...
read_flash (void)
{
    ...
    if (file_name != "")
    {
		...
		s << settings->getFlash(); 
        console->print (_("Reading ") + s.str() + _("KB of data from ROM to file: ") +
                        file_name + " ...");
		console->line();
        thread_RFLA->run ();
...
read_ram (void)
{
    ...
    if (file_name != "")
    {
		...
		std::ostringstream s (std::ostringstream::out);
		s << settings->getRam(); 
		console->print (_("Reading ") + s.str() + _("KB of data from RAM to file:\n") +
                        file_name);
        console->line();
