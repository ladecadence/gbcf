	...
	{			/* valid data packet */
	  ...
	  if (packet_number == packet[3]
		...
		if (fwrite (data, 1, sizeof data, file) < sizeof data)
	      ...
	      set_progress (recived, page_count * 256 - 1);
	    }
...
void
ReadFlashThread::set_progress (int p, int escala)
{
	if (escala > 1)
...
void ReadFlashThread::error(int err) {
	
	if (err =! true) {
		 std::cout << _(">> ERROR Read ROM: ") << err << std::endl;
		 exit(1);
	}
	else
